...
def get_download_command(download_link,download_path,filename,download_method,download_type="url"):
    ...
    if download_type == "url":
        if download_method == "more":
            command = "cd " + download_path + " && aria2c -s 16 -x 8 --seed-time=0 '" + download_link + "' -o " + filename + " && echo 下载完毕!文件已保存到" + download_path
        if download_method == "one":
            command = "cd " + download_path + " && aria2c --seed-time=0 '" + download_link + "' -o " + filename + " && echo 下载完毕!文件已保存到" + download_path
    
    if download_type == "cg":
        folder_name = download_link.split('/')[-2]  # 链接格式为 .../folder_name/filename
        # 构建下载命令
        command = "cd " + download_path + " && "
        # 根据下载方法添加下载命令
